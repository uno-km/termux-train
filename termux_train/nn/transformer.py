...
class TinyTransformerLM(Module):
    ...
    Full Decoder-Only Autoregressive Language Model:
      ...
      - Autoregressive generate() method with greedy / temperature sampling
    """
    ...
    def forward(
        ...
        backend = idx.backend
        # Generate position indices [0, 1, ..., S - 1]
        pos_list = list(range(S))
        pos_data = backend.from_data(pos_list, dtype="int64")
        pos_idx = Tensor(pos_data, dtype="int64", backend=backend)
        ...
        if targets is not None:
            # Flatten (B, S, V) and (B, S) for CrossEntropyLoss
    ...
    def generate(
        ...
        """
        Autoregressive next-token generation with greedy / temperature selection.
        """
