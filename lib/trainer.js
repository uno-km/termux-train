...
class TermuxTrainer extends EventEmitter {
  constructor(config = {}) {
    ...
    let heads = Number.isInteger(config.heads) && config.heads >= 1 ? config.heads : 2;

    // Strict division & RoPE invariants only apply to Transformer / Attention models
    if (modelType.includes('transformer') || modelType.includes('rope')) {
      if (!Number.isInteger(config.heads) || config.heads < 1) {
        // Adaptive default for transformer: 2 for even dim
        heads = (dim % 2 === 0) ? 2 : 1;
      }

      if (dim % heads !== 0) {
        throw new TypeError(
          `Transformer model dimension (dim=${dim}) is not divisible by heads (heads=${heads}). ` +
