  ...
  if (pyCmd) {
    try {
      ...
      const checkMod = spawnSync(
        pyCmd,
        ['-c', 'import termux_train; from termux_train import available_backends; print(",".join(available_backends()))'],
        { stdio: 'pipe', encoding: 'utf-8' }
      );
      if (!checkMod.error && checkMod.status === 0) {
        termuxTrainPyInstalled = true;
        const backends = checkMod.stdout.trim().toUpperCase().split(',');
        vulkanDetected = backends.includes('VULKAN');
      }
    } catch (_) {}
  }
